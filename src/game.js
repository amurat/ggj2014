(function(document) {
    ...
    GameLevel.prototype.create = function() {
        ...
        this.switchLevelButton = game.input.keyboard.addKey(Phaser.Keyboard.N);
        this.switchLevelButton.onDown.add(goToLevel, this);

        // Create Enemies

        var exclusionZoneSize = {
            width: 10,
            height: 10
        };
    ...
    GameLevel.prototype.speechUpdate = function() {
        var offset = {
            x: this.player1.body.width - this.speech1.body.width / 2.0 + 10,
            y: -this.player1.body.height / 2.0 - 10
        };
    ...
    GameLevel.prototype.endLevel = function() {
        if (this.health1 >= WIN_VALUE && this.health2 >= WIN_VALUE) {
            ...
            if (game.data.currentLevel < LEVEL_DATA.length) {
                ...
                game.state.add('loseScreen' + index, new MenuScreen('level' + game.data.currentLevel, false, 'You Failed To Keep Them Both Happy', 'Press Any Key to Retry Level.'));
                game.state.add('levelMenu' + index, new MenuScreen('level' + game.data.currentLevel, false, 'And The Concert Rages on... Part ' + game.data.currentLevel - LEVEL_DATA.length + 1, 'Press Any Key to Continue.'));
                game.state.add('level' + index, new GameLevel(), false);
            }
        }
        if (this.health1 <= LOSE_VALUE || this.health2 <= LOSE_VALUE) {
            this.mainCharVoice.stop();
            game.state.start('loseScreen' + game.data.currentLevel);
        }
    };
    //////////////////////////////////////////////////////////////////////////
    ...
    LEVEL_DATA.forEach(function(ELEMENT, index) {
        game.state.add('loseScreen' + index, new MenuScreen('level' + index, false, 'You Failed To Keep Them Both Happy', 'Press Any Key to Retry Level.'));
